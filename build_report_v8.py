...
m_kg = 25.093; g_acc = 9.8; W_N = m_kg * g_acc; r_m = 0.6513; M_J = W_N * r_m
...
def build():
    ...
    B(doc, f'振り子型シャルピー試験機を使用した．試験機定数：m={m_kg}kg，g={g_acc}m/s²，'
      f'W=mg={W_N:.1f}N，r={r_m}m，M=Wr={M_J:.2f}J，α={alpha_deg}°．')
    ...
    proc(doc, '④ 損失エネルギーLの測定（試験片なしで100往復）')

    # --- 3-4 結果 ---
    section(doc, '3-4　結果')
    B(doc, 'シャルピー吸収エネルギーKは次式で算出する．')
    equation(doc, 'K = M (cosβ − cosα) − L', eq())
    equation(doc, 'ρ = K / A', eq())
    B(doc, f'ここでM={M_J:.2f}J，cosα=cos{alpha_deg}°={cos_alpha:.4f}，Lは試験片なし100往復による損失エネルギー[J]である．')
    ...
    tn6 = tn()
    tbl_caption(doc, tn6, 'シャルピー試験結果')
    make_table(doc,
        ['材料', 'β [°]', 'K₀ [J]', 'K [J]', 'A [cm²]', 'ρ [J/cm²]'],
        [('SS400', f'{CHARPY_BETA["SS400"]}', f'{ss_K0:.2f}', f'{ss_K0:.2f}−L', '1.000', f'({ss_K0:.2f}−L)/1.000'),
         ('FC250', f'{CHARPY_BETA["FC250"]}', f'{fc_K0:.2f}', f'{fc_K0:.2f}−L', '0.997', f'({fc_K0:.2f}−L)/0.997'),
         ('A7075', f'{CHARPY_BETA["A7075"]}', f'{a7_K0:.2f}', f'{a7_K0:.2f}−L', '0.990', f'({a7_K0:.2f}−L)/0.990')])
    Bx(doc, '※ K₀ = M(cosβ−cosα)（損失補正前）．損失エネルギーLの計測値が得られ次第，Kおよびρを表6に追記する．')
    Bx(doc, '')
    ...
    B(doc, f'引張試験から得た延性序列（SS400: δ={KV["SS400"]["delta"]:.2f}% >> A7075: {KV["A7075"]["delta"]:.2f}% > FC250: {KV["FC250"]["delta"]:.2f}%）'
      'は，シャルピー試験での吸収エネルギーの序列とも概ね対応する．'
      f'SS400は延性が大きく塑性変形によるエネルギー吸収が大きいため，K₀={ss_K0:.2f}−L Jと最大の吸収エネルギーが期待される．'
      f'FC250は脆性的で塑性変形が小さく，K₀={fc_K0:.2f}−L Jと最小の吸収エネルギーとなると予測される．'
      f'A7075はその中間でK₀={a7_K0:.2f}−L Jと推測される．'
      '損失エネルギーLが確定した後，各材料のKおよびρを比較することで定量的に確認できる．')

    subsection(doc, '課題14　脆性破面率と吸収エネルギーの関係')
    B(doc, '脆性破面率B [%]は，破面全面積に対する脆性的に破壊した領域の割合であり，材料の破壊様式を定量化する指標の一つである．'
      '一般にBが大きい材料ほどシャルピー吸収エネルギーが小さい傾向がある．'
      '延性破壊ではき裂先端に塑性変形帯が発達してエネルギー吸収量が増加するが，'
      '脆性破壊では塑性変形を伴わないためエネルギー吸収量が小さい．'
      'FC250は黒鉛フレークを起点とした脆性的破壊が支配的であり，Bが高く吸収エネルギーが小さいと予測される．'
      'SS400では破面の大部分が延性的（ディンプル）であるためBが低く吸収エネルギーが大きい．'
      'β角測定後にK・ρを算出し，BとρのBetween-material相関を考察する．')
